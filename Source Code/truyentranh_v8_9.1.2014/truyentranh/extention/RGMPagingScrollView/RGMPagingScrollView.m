...
- (CGSize)contentSizeForDirection:(RGMScrollDirection)direction
{
    switch (direction) {
        case RGMScrollDirectionHorizontal:
            return CGSizeMake((self.pageWidth + self.gutter) * self.numberOfPages, 1);
            break;
        case RGMScrollDirectionVertical:
            return CGSizeMake(1, (self.pageHeight + self.gutter) * self.numberOfPages + 200);
            break;
        default:
...
- (NSInteger)currentPage
{
    ...
    switch (self.scrollDirection) {
        case RGMScrollDirectionHorizontal: {
            ...
            currentPage = roundf(CGRectGetMinX(self.bounds) / pageWidth);
            break;
        }
        case RGMScrollDirectionVertical: {
            CGFloat pageHeight = self.viewModel.pageHeight + self.viewModel.gutter;
            currentPage = roundf(CGRectGetMinY(self.bounds) / pageHeight);
            break;
        }
    }
